...
suspend fun invokeBollyflix(
    ...
    val url = if (season == null) {
        ...
        "$api/search/$fixTitle $season"
    }

    val searchResult = app.get(url).document.select("#content_box article").toString()
    val hrefPattern = Regex("""(?i)<article[^>]*>\s*<a\s+href="([^"]*$searchTitle[^"]*)"""")
        .find(searchResult)?.groupValues?.get(1)
    ...
    val entries = response.select("div.thecontent $hTag:matches((?i)$sTag.*(720p|1080p|2160p))")
        .filter { element ->
            !element.text().contains("Bollyflix", true) &&
                    !element.text().contains("1080p", true) || !element.text().contains("720p", true)
        }

    entries.amap { entry ->
        val tags = """(?:480p|720p|1080p|2160p)(.*)""".toRegex()
            .find(entry.text())?.groupValues?.get(1)?.trim()

        val quality = """480p|720p|1080p|2160p""".toRegex()
            .find(entry.text())?.groupValues?.get(0)?.trim()

        var href = entry.nextElementSibling()
            ?.select("a:contains($aTag)")?.attr("href")
            ?.substringAfter("=") ?: ""

        href = base64Decode(href)

        val selector = if (season == null) "p a.maxbutton" else "h3 a:matches(Episode $episode)"
        if (href.isNotEmpty()) {
            app.get(href).document.selectFirst(selector)?.let {
                val link = it.attr("href")
                if (link.contains("driveseed.org")) {
                    val file = app.get(link).toString()
                        .substringAfter("replace(\"").substringBefore("\")")
                    val domain = getBaseUrl(link)
                    val server = "$domain$file"
                    loadAddSourceExtractor("Bollyflix", server, "", subtitleCallback, callback)
                }
                val server = bypassHrefli(link) ?: ""
                if (server.isNotEmpty()) {
                    loadAddSourceExtractor("Bollyflix", server, "", subtitleCallback, callback)
