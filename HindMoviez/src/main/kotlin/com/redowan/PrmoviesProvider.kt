...
class Prmovies : MainAPI() {
    override var mainUrl = "https://prmovies.design/"
    ...
    override val mainPage = mainPageOf(
        "genre/dual-audio/" to "Dual Audio",
        "/genre/bollywood/" to "Bollywood",
        "/genre/hollywood/" to "Hollywood",
        "/series/" to "English Series",
        "/web-series/" to "Hindi Series",
        )
    override suspend fun getMainPage(
        ...
        val home = document.select(".ml-item").mapNotNull {
            it.toSearchResult()
        }
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        val title = this.selectFirst("span.mli-info h2")?.text()?.trim() ?: return null
        val href = fixUrl(this.selectFirst(".ml-mask jt")?.attr("href").toString())
        val posterUrl = fixUrlNull(this.selectFirst(".ml-mask img")?.attr("src"))
        val quality = getQualityFromString(this.select(".mli-quality span").text().substringBefore("-"))
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select(".ml-item").mapNotNull {
            it.toSearchResult()
        }
    }
//
